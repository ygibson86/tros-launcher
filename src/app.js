...
// ---------- Пикер эмодзи ----------
function buildEmojiPicker(containerEl, targetInputEl) {
...
function toggleEmojiPicker(containerEl) {
  containerEl.classList.toggle("open");
}

document.getElementById("btnIconPickerToggle").addEventListener("click", () => {
  const picker = document.getElementById("btnEmojiPicker");
  buildEmojiPicker(picker, document.getElementById("btnIconInput"));
  toggleEmojiPicker(picker);
});

document.getElementById("appLogoPickerToggle").addEventListener("click", () => {
  const picker = document.getElementById("appEmojiPicker");
  buildEmojiPicker(picker, document.getElementById("appLogoInput"));
  toggleEmojiPicker(picker);
});
...
async function applyBranding() {
  ...
  document.getElementById("brandMark").textContent = logo;
  document.title = name;
  try {
...
function renderSearchResults(query) {
  ...
  state.config.groups.forEach((g) => {
    g.buttons.forEach((b) => {
      const hay = (b.name + " " + (b.command || "")).toLowerCase();
...
function createTile(btn, group, opts) {
  const firstLine = (btn.command || "").split(/\r?\n/)[0];
  const isMultiline = (btn.command || "").includes("\n");
  const preview = firstLine + (isMultiline ? " …" : "");
  ...
  tile.innerHTML = `
    ...
    <div class="tile-icon">${btn.icon ? escapeHtml(btn.icon) : "▶"}</div>
    <div class="tile-name">${escapeHtml(btn.name)}</div>
    <div class="tile-cmd">${escapeHtml(preview)}</div>
...
function openButtonEditor(group, button) {
  ...
  document.getElementById("btnEmojiPicker").classList.remove("open");
...
document.getElementById("settingsBtn").addEventListener("click", () => {
  ...
  document.getElementById("appEmojiPicker").classList.remove("open");
